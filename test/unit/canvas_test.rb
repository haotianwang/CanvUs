...
class CanvasTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    Canvas.create(canvas_id: 0, bitmap_id: bitmap0[:id], user_username: 'King Altruism', active: true)
    Canvas.create(canvas_id: 1, bitmap_id: bitmap1[:id], user_username: 'King Altruism', active: true)
    Canvas.create(canvas_id: 2, bitmap_id: bitmap2[:id], user_username: 'King Altruism', active: true)
  end
  ...
  def testCanvasValidations
    ...
    Canvas.create(bitmap_id: 1, user_username: 'HaotianWang92', active: true)
    Canvas.create(canvas_id: 3, user_username: 'HaotianWang92', active: true)
    Canvas.create(canvas_id: 4, bitmap_id: 2, active: true)
    assert_equal(numCanvases, Canvas.all.count, "Canvases with empty fields were incorrectly inserted")
  end

  # Test that canvas_id is actually a primary key, preventing duplicate canvas_id field values.
  def testCreateCanvasDuplicateID
    numCanvases = Canvas.all.count
    assert_raise(ActiveRecord::RecordNotUnique) { Canvas.createCanvas(0, 1) }
    assert_equal(numCanvases, Canvas.all.count, "Canvas with a duplicate id was incorrectly stored")
  end
  ...
  def testSetBitmap
    canvas = Canvas.where("canvas_id = ?", 0).first
    originalBitmapID = canvas[:bitmap_id]
    Canvas.setBitmap(canvas[:canvas_id], 3)
    canvas = Canvas.where("canvas_id = ?", 0).first
  ...
  def testSetActivity
    ...
    Canvas.setActivity(canvas[:canvas_id], !originalActivity)
    canvas = Canvas.where("canvas_id = ?", 0).first
  ...
  def testGetCanvasIDs
    ...
    assert_equal("0|1|2", canvasIDs, "Canvas ids were not correctly retrieved/output")
    Canvas.create(canvas_id: 3, bitmap_id: 1, user_username: 'mylittlelucas888', active: false)
